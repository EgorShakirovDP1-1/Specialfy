                                        ...
                                        <button v-if="$page.props.auth"
                                            ...
                                            <div class="d-flex align-items-center m-0">
                                                ...
                                                <i class="bi h4 text-danger ms-1 mt-2"
                                                    :class="{ 'bi bi-hand-thumbs-up-fill': film.isLikedByUser, 'bi bi-hand-thumbs-up': !film.isLikedByUser }"></i>
                                            </div>
                                        </button>
                                        <button v-if="$page.props.auth"
                                            class="btn btn-light border-none px-2 py-0 btn-48"
                                            @click="toggleDislike(film.id)">
                                            <div class="d-flex align-items-center m-0">
                                                {{ film.dislikesCount }}
                                                <i class="bi h4 text-danger ms-1 mt-2"
                                                    :class="{ 'bi bi-hand-thumbs-down-fill': film.isDislikedByUser, 'bi bi-hand-thumbs-down': !film.isDislikedByUser }"></i>
                                            </div>     
                                        </button>

                                        
                                        <Link v-if="!$page.props.auth" :href="route('login')">
                                        <button class="btn btn-light border-none px-2 py-2 btn-48 bg-green">
                                            <div class="d-flex align-items-center m-0">
                                                {{ film.raiting }}
                                                <i class="bi bi-hand-thumbs-down h4 text-danger ms-1 mt-2"></i>
                                            </div>
                                        </button>
                                        </Link>
                                    </div>
...
export default {
    ...
    methods: {
    async toggleLike(filmId) {
        console.log("Like button clicked for film ID:", filmId); // Log button click
        if (this.isLoading) {
            console.log("Already processing a request, ignoring click."); // Log if already loading
        ...
        if (film) {
            ...
            film.likesCount += film.isLikedByUser ? 1 : -1;
            console.log("Optimistic update for film:", film);
        }

        try {
            await this.$inertia.post(`/films/${filmId}/like`, {}, { preserveScroll: true });
            console.log("Successfully toggled like for film ID:", filmId);
        } catch (error) {
    ...
    async toggleDislike(filmId) {
        console.log("Dislike button clicked for film ID:", filmId); // Log button click
        if (this.isLoading) {
            console.log("Already processing a request, ignoring click."); // Log if already loading
            return; // Ignore if already loading
        }
        ...
        const film = this.films.find(f => f.id === filmId);
        if (film) {
            film.isDislikedByUser = !film.isDislikedByUser;
            film.dislikesCount += film.isDislikedByUser ? 1 : -1;
            console.log("Optimistic update for film:", film);
        }

        try {
            await this.$inertia.post(`/films/${filmId}/dislike`, {}, { preserveScroll: true });
            console.log("Successfully toggled dislike for film ID:", filmId);
        } catch (error) {
